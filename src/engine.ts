...
const baseLink =
	"https://github.com/wxn0brP/Zhiva-native/releases/download/native/";
...
async function downloadEngine(
	...
	if (os !== "win32") await $`chmod +x ${engineFilePath}`;
	...
	log(
		LogLevel.INFO,
		"05-22",
		`Zhiva for ${os} downloaded successfully (v${serverVersion.trim()})`,
	);
}
	...
	try {
		...
		if (binaryExists && localVersion === serverVersion.trim()) {
			log(
				LogLevel.INFO,
				"05-10",
				`Zhiva engine for ${os} is up to date (v${serverVersion.trim()})`,
			);
			return true;
		}
		...
		if (!binaryExists) {
			...
			log(
				LogLevel.INFO,
				"05-12",
				`Version mismatch for ${os} (local: ${localVersion} != server: ${serverVersion.trim()}), updating...`,
			);
		}
...
export async function checkEngines(config: Config): Promise<void> {
	...
	if (platforms.length === 0) {
		log(
			LogLevel.INFO,
			"05-01",
			"No platforms enabled in config, skipping engine check",
		);
		return;
	}
	...
	log(
		LogLevel.IMPORTANT,
		"05-02",
		`Checking Zhiva engines for: ${platforms.join(", ")}`,
	);
	...
	try {
		...
		for (const os of platforms) {
			...
			if (!existsSync(engineFilePath))
				console.warn(
					`[Z-BLD-05-32] Cannot verify ${os}: server unreachable AND binary missing`,
				);
		}
	}
}
